    ...
    def loss(self, input, target_is_real, for_discriminator=True):
        ...
        if self.gan_mode == 'original':  # cross entropy loss
            ...
            if for_discriminator:
                if target_is_real:
                    minval = torch.min(
                        input - 1, self.get_zero_tensor(input).to(device_))
                    loss = -torch.mean(minval)
                else:
                    minval = torch.min(-input - 1,
                                       self.get_zero_tensor(input).to(device_))
                    loss = -torch.mean(minval)
            else:
...
class VGGLoss(nn.Module):
    ...
    def forward(self, x, y, mask=None):
        ...
        #     y[:,c,:,:] = (y[:,c,:,:]-self.mean[c])/self.std[c]
        ...
        if mask is None:
            ...
            for i in range(len(x_vgg)):
                ...
                loss += self.weights[i] * \
                    self.criterion(x_vgg[i]*m, y_vgg[i].detach()*m)
            return loss
...
class MaskedVGGLoss(nn.Module):
    ...
    def forward(self, x, y, mask):
        ...
        for i in range(len(x_vgg)):
            b, _, h, w = x_vgg[i].shape
            m = F.interpolate(mask, size=(b, 1, h, w))
            loss += self.weights[i] * \
                self.criterion(x_vgg[i]*m, y_vgg[i].detach()*m)
        return loss
